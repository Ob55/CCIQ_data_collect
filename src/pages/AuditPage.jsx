...
export function AuditPage() {
  ...
  const { data: rows, loading, error } = useAsync(() => listAudit(filters), [filters])
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={onFilter} className="flex flex-wrap items-end gap-3 rounded-md border p-3">
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          Action
          <Input value={draft.action} onChange={set('action')} placeholder="e.g. review" className="h-9" />
        </label>
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          Entity
          <select value={draft.entityType} onChange={set('entityType')} className={SELECT}>
        ...
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          From
          <input type="date" value={draft.from} onChange={set('from')} className={SELECT} />
        </label>
        <label className="flex flex-col gap-1 text-xs text-muted-foreground">
          To
          <input type="date" value={draft.to} onChange={set('to')} className={SELECT} />
        </label>
