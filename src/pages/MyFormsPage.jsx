...
// Deployed forms this user is assigned to fill (§7). Admins see every deployed form.
export function MyFormsPage() {
  const { data: forms, loading, error } = useAsync(() => listFillableForms(), [])
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {forms.map((f) => (
            <Link key={f.id} to={`/f/${f.slug}`} className="group">
