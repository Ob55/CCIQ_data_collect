...
/**
 * Left sidebar navigation. Server-rendered role filtering happens in the layout; this client
 * component only handles active-link highlighting and the mobile drawer.
 *
 * @param {{
 ...
 * }} props
 */
export function SidebarNav({ sections, user, logout }) {
    ...
    <>
      ...
      <aside className="hidden w-64 shrink-0 flex-col bg-brand text-brand-foreground md:sticky md:top-0 md:flex md:h-screen">
        <div className="px-5 py-5">
          <BrandLogo width={150} height={45} priority />
        </div>
        {nav}
        {footer}
      </aside>
      ...
      <header className="flex items-center justify-between bg-brand px-4 py-3 text-brand-foreground md:hidden">
        <BrandLogo width={130} height={39} priority />
        <button
          type="button"
