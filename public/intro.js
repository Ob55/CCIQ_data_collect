(function () {
  ...
  var tagPaths = (D.tag.glyphs || []).map(function (g) { return new Path2D(g.d); });
  ...
  /* the descriptor under the wordmark — set in the interface face rather
     than the logotype, tracked wide, and resolving as the network completes */
  function tagline(t) {
    var txt = CC_CONFIG.tagline;
    if (!txt) return;
    var G = D.tag;

    // the configured wording is pre-outlined in the same font as the logo
    // files, so it needs no font at all and matches the asset exactly
    if (txt === G.text && tagPaths.length) {
      ctx.save();
      ctx.fillStyle = ink;
      for (var i = 0; i < tagPaths.length; i++) {
        var p = cl((t - (G.t + G.glyphs[i].k * G.step)) / G.dur);
        if (p <= 0) continue;
        var e = out3(p);
        ctx.save();
        ctx.globalAlpha = cl(p * 1.6) * G.alpha;
        ctx.translate(0, 11 * (1 - e));
        ctx.fill(tagPaths[i]);
        ctx.restore();
      }
      ctx.restore();
    ...
    // changed wording falls back to the interface font
    ctx.save();
    ctx.font = G.family;
    if (String(ctx.font).indexOf(G.size + 'px') === -1) {
      ctx.font = (G.weight || 700) + ' ' + G.size + 'px sans-serif';
    }
    ctx.textBaseline = 'alphabetic';
    var ch = String(txt).split('');
    var w = ch.map(function (c) { return ctx.measureText(c).width; });
    var total = w.reduce(function (a, b) { return a + b; }, 0) + G.track * (ch.length - 1);
    var x = G.cx - total / 2;
    ctx.fillStyle = ink;
    for (var j = 0; j < ch.length; j++) {
      var q = cl((t - (G.t + j * G.step)) / G.dur);
      if (q > 0 && ch[j] !== ' ') {
        var e2 = out3(q);
        ctx.globalAlpha = cl(q * 1.6) * G.alpha;
        ctx.fillText(ch[j], x, G.y + 11 * (1 - e2));
      }
      x += w[j] + G.track;
    }
    ctx.restore();
  ...
  function ready() {
    ...
    try { parent.postMessage('cleancookiq:ready', '*'); } catch (e) {}
  }
